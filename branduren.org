...
#+begin_src emacs-lisp
  (use-package avy
    ...
    :init
    (setq avy-keys
          '(?h ?u ?t ?e ?n ?o ?s ?a ?d ?i ?g ?p ?c ?r ?b ?k ?y ?f)))
#+end_src
...
**** Org-mode-hook set key
#+begin_src emacs-lisp
  (add-hook 'org-mode-hook
            '(lambda()
               (local-set-key (kbd "C-c t") 'org-babel-hide-result-toggle)
               (local-set-key (kbd "C-c d") 'org-babel-remove-result)))
#+end_src

**** Org-mode-hook unset key
#+begin_src emacs-lisp
  (add-hook 'org-mode-hook
            '(lambda()
...
#+begin_src emacs-lisp
  ...
  (defvar my-mu4e-account-alist
    '(("ktbit"
       ...
       (user-mail-address "brandur@ktbit.com"))
      ("tekno"
       (mu4e-sent-folder "/tekno/sent")
       (mu4e-drafts-folder "/tekno/drafts")
       (mu4e-trash-folder "/tekno/trash")
...
#+begin_src emacs-lisp
  ...
  (add-hook 'message-mode-hook
            '(lambda()
               (local-set-key (kbd "C-c C-w") 'myfunc-mail-with-signature)))
#+end_src
...
Rememer to unset "M-h" in org-mode [[Org-mode-hook unset key]]
#+begin_src emacs-lisp
  ;; delete with ease
...
#+begin_src emacs-lisp
    (use-package selected
      ...
      :bind (:map selected-keymap
                  ...
                  ("v" . myfunc-view-region-in-new-buffer)
                  :map selected-org-mode-map
                  ("t" . org-table-convert-region)))
#+end_src

**** View region in new buffermyfunc
A function I created which inserts the region into a new buffer. It works perfectly with selected-minor-mode.

#+begin_src emacs-lisp
  (defun myfunc-view-region-in-new-buffer (beg end &optional arg)
    "Copy region to new buffer and switch to that buffer in other-window"
    (interactive "*r\nP")
    (if (> beg end) (let (mid) (setq mid beg beg end end mid)))
    (let ((buffer-name (buffer-substring beg (+ beg 10)))
          (mode major-mode))
      (generate-new-buffer buffer-name)
      (copy-to-buffer buffer-name beg end)
      (save-excursion
...
My first hydra function. It's nice to get an overview.

#+begin_src emacs-lisp
    (bind-key "<f12>" 'myfunc/hydra-networking/body)
    (defhydra myfunc/hydra-networking (:color blue :hint nil)
      "
    Network utils^^
    ------------------------------------
    i_f_config
    ...
    _d_ig
  "
      ("f" (lambda () (interactive) (ifconfig) (other-window 1)))
      ("w" (lambda () (interactive) (iwconfig) (other-window 1)))
      ("e" (lambda () (interactive) (netstat) (other-window 1)))
      ("a" (lambda () (interactive) (arp) (other-window 1)))
      ("r" (lambda () (interactive) (route) (other-window 1)))
      ("t" traceroute)
      ("p" ping)
      ("s" nslookup)
      ("d" run-dig)
...
(print-elements-of-list (network-interface-list))

(defun ip-list ()
    (let ((ilist (network-interface-list))
          (iplist '()))
      (while ilist
        (let ((i 0)
              (device (car (car ilist)))
              (ipv (cdr (car ilist))))
          (setq ip "")
          (while (< i (- (length ipv) 1))
            (setq ip (concat ip (format "%d" (elt ipv i))))
            (if (< i 3)
                (setq ip (concat ip ".")))
            (setq i (1+ i)))
          (message (concat device " " ip))
          (setq iplist (append iplist (list (list device ip)))))
...
(apply 'concat (network-interface-list))

(nmap "sP")
(message (car (cdr(cdr (network-interface-info "lo"))))
(message (get-ip-address))
(message (get-ip-address "wlp3s0"))
(message dev)
...
(simple-mpc-call-mpc t "play")

(simple-mpc-call-mpc t (list "load" "music/Brandur/5-vem-kan-segla-forutan-vind.ogg")

(simple-mpc-call-mpc t (list "ls" "music/Brandur/"))

(simple-mpc-call-mpc t (list "lsplaylists"))



(simple-mpc-call-mpc t (list "move" "1" "2"))

(simple-mpc-call-mpc t (list "save" "queue"))

(simple-mpc-call-mpc t "save")
