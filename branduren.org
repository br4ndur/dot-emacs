...
Load require usepackage

#+begin_src emacs-lisp
(add-to-list 'load-path "~/.emacs.d/elpa/org-plus-contrib-20141201")
(require 'use-package)
#+end_src
...
*** Auto complete
#+begin_src emacs-lisp
  (myfunc/ensure-package-installed 'auto-complete)
  (auto-complete-mode t)
#+end_src

#+begin_src emacs-lisp
   ;; Completion words longer than 4 characters
   (myfunc/ensure-package-installed 'ac-ispell)
...
#+begin_src emacs-lisp
  (myfunc/ensure-package-installed 'auto-complete-c-headers)
...
#+begin_src emacs-lisp
  (myfunc/ensure-package-installed 'evil-numbers)
...
#+begin_src emacs-lisp
    (myfunc/ensure-package-installed '(web-mode))
...
#+b_src emacs-lisp
;;U different shell command for mix.
(selchemist-mix-command "/usr/bin/mix")

;;U different task for running tests.
(selchemist-mix-test-task "espec")

;;Uustom mix test task options.
(selchemist-mix-test-default-options '()) ;; default

;;U different environment variable in which mix tasks will run.
(selchemist-mix-env "prod")
#+erc

iex

#+b_src emacs-lisp
;;U different shell command for iex.
(selchemist-iex-program-name "/usr/bin/iex") ;; default: iex
...
#+b_src emacs-lisp
;; a different shell command for elixir.
(selchemist-execute-command "/usr/bin/elixir") ;; default: elixir
...
#+b_src emacs-lisp
;; a different shell command for elixirc.
(selchemist-compile-command "/usr/bin/elixirc") ;; default: elixirc
...
#+b_src emacs-lisp
;; ble the change of the modeline color with the last test run status.
(selchemist-test-status-modeline nil)
...
#+b_src emacs-lisp
;;U different keybinding prefix than C-c a
(selchemist-key-command-prefix (kbd "C-,")) ;; default: (kbd "C-c a")
#+erc

***ting Mode

#+b_src emacs-lisp
;; ble the use of a more significant syntax highlighting on functions like test, assert_* and refute_*
(selchemist-test-mode-highlight-tests nil) ;; default t

;; t ask to save changed file buffers before running tests.
(selchemist-test-ask-about-save nil)

;; t change the color of the mode-name when test run failed or passed.
(selchemist-test-status-modeline nil)

;;Scompilation output in test report.
(selchemist-test-display-compilation-output t)

#+erc

***ks
#+b_src emacs-lisp
;; the whole test suite with alchemist-mix-test after saving a buffer.
(selchemist-hooks-test-on-save t)
...
** ESS
#+begin_src emacs-lisp
  (myfunc/ensure-package-installed '(ess-R-data-view
                                        ess-R-object-popup
...
#+begin_src emacs-lisp
(myfunc/ensure-package-installed 'erc-hl-nicks)
...
#+begin_src emacs-lisp
(myfunc/ensure-package-installed 'helm-emmet)
